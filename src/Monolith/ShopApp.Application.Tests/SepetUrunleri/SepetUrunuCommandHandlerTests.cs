...
public class SepetUrunuCommandHandlerTests
{
    ...
    private static SepetUrunu CreateItemWithParent(Guid musteriId)
    {
        ...
        var urun = SepetUrunu.Olustur(sepet.Id, Guid.NewGuid(), 2, 2, 15m, musteriId);
    ...
    public async Task Create_Throws_WhenParentCartBelongsToAnotherCustomer()
    {
        ...
        await Assert.ThrowsAsync<KeyNotFoundException>(() =>
            handler.Handle(new CreateSepetUrunuCommand(sepet.Id, Guid.NewGuid(), 1, 1, 10m), CancellationToken.None));
    ...
    public async Task Create_Succeeds_WhenParentCartBelongsToCaller()
    {
        ...
        var id = await handler.Handle(new CreateSepetUrunuCommand(sepet.Id, Guid.NewGuid(), 1, 1, 10m), CancellationToken.None);
    ...
    public async Task Update_Throws_WhenItemBelongsToAnotherCustomersCart()
    {
        ...
        await Assert.ThrowsAsync<KeyNotFoundException>(() =>
            handler.Handle(new UpdateSepetUrunuCommand(urun.SepetId, urun.Id, 5, 5), CancellationToken.None));
    ...
    public async Task Update_Succeeds_WhenItemBelongsToCallersCart()
    {
        ...
        await handler.Handle(new UpdateSepetUrunuCommand(urun.SepetId, urun.Id, 5, 5), CancellationToken.None);
