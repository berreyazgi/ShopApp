...
namespace ShopApp.Application.Tests.SiparisUrunu;
...
public class SiparisUrunuCommandHandlerTests
{
    ...
    private static SiparisUrunleri CreateItemWithParent(Guid musteriId)
    {
        var siparis = SiparisEntity.Olustur(musteriId, "SIP-ITEM-TEST", Guid.NewGuid());
        var urun = SiparisUrunleri.Olustur(siparis.Id, Guid.NewGuid(), "Test Ürünü", null, null, 2, 25m, 0m, musteriId);
        PrivateNavigation.Set(urun, nameof(SiparisUrunleri.SiparisEntity), siparis);
        return urun;
    }
    ...
    public async Task Create_Throws_WhenParentOrderBelongsToAnotherCustomer()
    {
        ...
        urunuRepository.Verify(r => r.AddAsync(It.IsAny<SiparisUrunleri>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task Create_Succeeds_WhenParentOrderBelongsToCaller()
    {
        ...
        urunuRepository.Verify(r => r.AddAsync(It.IsAny<SiparisUrunleri>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Update_Throws_WhenItemBelongsToAnotherCustomersOrder()
    {
        ...
        repository.Verify(r => r.UpdateAsync(It.IsAny<SiparisUrunleri>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task Delete_Throws_WhenItemBelongsToAnotherCustomersOrder()
    {
        ...
        repository.Verify(r => r.DeleteAsync(It.IsAny<SiparisUrunleri>(), It.IsAny<CancellationToken>()), Times.Never);
    }
